 ...
 * e.g., scalar types, sub-range types, products of types, function types,
...
public abstract class Type {
    ...
    /**
     * All types require space to be allocated (maybe 0)
     */
    int space;
     ...
     * @throws IncompatibleTypes if it cannot coerce to 'this' type
     */
    public ExpNode coerceToType(ExpNode exp) throws IncompatibleTypes {
        ...
        if (!this.equals(fromType) && fromType != ERROR_TYPE) {
            ...
            try {
                ...
                errors.debugMessage("Failed to coerce " + newExp + " to " + this.getName());
                errors.decDebug();
    ...
    /**
     * If 'this' is a sub-range type widen it to its base type.
    ...
    /**
     * Types defined as a sub-range of a scalar type.
     */
    public static class SubrangeType extends ScalarType {
        /**
         * The base type of the sub-range type
         */
        ...
        /**
         * Basic constructor for a sub-range type.
         * @param loc location of its definition
         * @param lowerExp constant expression for lower bound of sub-range
         * @param upperExp constant expression for upper bound of sub-range
         */
        public SubrangeType(Location loc, ConstExp lowerExp, ConstExp upperExp) {
             ...
             * just fit the sub-range, e.g., a sub-range of 0..255
        ...
        /**
         * Coerce expression to 'this' sub-range type
         * The objective is to create an expression of the 'this' sub-range type
         * from exp. Note that this method is called from coerceToType, which
         * handles optional dereference of a type and the simple cases when
         * the two types are equal or one is error type.
         *
         ...
         * @return the coerced expression
         * @throws IncompatibleTypes exception if it is not possible to coerce
         *                           exp to 'this' sub-range type
         */
        @Override
        protected ExpNode coerce(ExpNode exp) throws IncompatibleTypes {
            /* This implements Rule Narrow sub-range in the static semantics.
             * If the types do not match, we can try coercing the expression
             * to the base type of 'this' sub-range, and then narrow that
             ...
             * type of 'this' sub-range type. We just need to narrow it
             * down to 'this' sub-range.
             */
        ...
        /**
         * Resolving a sub-range type requires the lower and upper bound
        ...
        public Type resolveType() {
            if (!resolved) {
                ...
                if (!upperExp.getType().equals(lowerExp.getType())) {
                    errors.error("Types of bounds of sub-range must match", loc);
                    baseType = ERROR_TYPE; // TODO or leave as lower type?
                } else if (upper < lower) {
                    errors.error("Upper bound of sub-range less than lower bound", loc);
                }
                resolved = true;
            }
            return this;
        ...
        /**
         * A sub-range type is equal to another sub-range type only if they have
         * the same base type and lower and upper bounds.
         */
        @Override
        public boolean equals(Type other) {
            if (other instanceof SubrangeType otherSubrange) {
    ...
    public static class ProductType extends Type {
        ...
        @Override
        public boolean equals(Type other) {
            if (other instanceof ProductType otherProduct) {
                List<Type> otherTypes = otherProduct.getTypes();
    ...
    public static class FunctionType extends Type {
        ...
        @Override
        public boolean equals(Type other) {
            if (other instanceof FunctionType otherFunction) {
     ...
     * type that is a subtype of all the types. If x is of the intersection
    ...
    public static class IntersectionType extends Type {
         ...
         */
        void addType(Type t) {
            if (t instanceof IntersectionType intersectionType) {
                types.addAll(intersectionType.getTypes());
            } else {
                types.add(t);
            }
        }
        ...
        @Override
        public boolean equals(Type other) {
            if (other instanceof IntersectionType otherIntersection) {
                List<Type> otherTypes = otherIntersection.getTypes();
                if (types.size() == otherTypes.size()) {
                    for (Type t : types) {
        ...
        protected ExpNode coerce(ExpNode exp) throws IncompatibleTypes {
            ...
            errors.decDebug();
            /* If we get here, we were unable to coerce exp to any one of
    ...
    public static class IdRefType extends Type {
        ...
        /**
         * Resolved real type, or ERROR_TYPE if it cannot be resolved.
         */
    ...
    public static class ReferenceType extends AddressType {
        ...
        @Override
        public boolean equals(Type other) {
            return other instanceof ReferenceType otherRef &&
                    otherRef.getBaseType().equals(this.getBaseType());
        }
