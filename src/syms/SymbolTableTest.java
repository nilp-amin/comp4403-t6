...
public class SymbolTableTest extends TestCase {
    ...
    public void testAddVariable() {
        Type intType = Predefined.INTEGER_TYPE;
        Type refInt = new Type.ReferenceType(ErrorHandler.NO_LOCATION, intType);
        assertNull(currentScope.lookupVariable("e"));
        currentScope.addVariable("e", new Location(0, 0), intType);
        currentScope.resolveScope();
        SymEntry.VarEntry e = currentScope.lookupVariable("e");
        checkVariable(e, "e", 1, refInt, 0, 1);

        currentScope.addVariable("f", new Location(0, 0), intType);
        currentScope.resolveScope();
        SymEntry.VarEntry f = currentScope.lookupVariable("f");
        checkVariable(f, "f", 1, refInt, 1, 2);

        currentScope.addVariable("g", new Location(0, 0), intType);
        currentScope.resolveScope();
